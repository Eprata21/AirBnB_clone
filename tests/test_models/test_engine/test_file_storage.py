...
class TestFileStorage(unittest.TestCase):
    ...
    def test_all_cls(self):
        ...
        try:
            ob = self.storage.all(BaseModel)
            self.assertEqual(type(ob), dict)
            self.assertEqual(len(ob), 1)
            self.assertEqual(self.base, list(ob.values())[0])
        except TypeError:
            pass
    ...
    def test_new(self):
        ...
        self.assertIn("BaseModel." + b.id, store.keys())
    ...
    def test_reload(self):
        ...
        with open("file.json", "w", encoding="utf-8") as f:
            ...
            json.dump({key: b.to_dict()}, f)
        self.storage.reload()
        store = FileStorage._FileStorage__objects
        self.assertIn("BaseModel." + b.id, store)
